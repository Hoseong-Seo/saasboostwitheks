...
import software.amazon.awssdk.services.codepipeline.model.StartPipelineExecutionResponse; //EKS-Boost
import software.amazon.awssdk.services.codepipeline.model.TagResourceRequest; //EKS-Boost
...
public class OnboardingService {
    ...
    /**
     * Get an onboarding record by id. Integration for GET /api/tenants/auth- endpoint.
     ...
     */
    //EKS 
    public APIGatewayProxyResponseEvent getOnboardingTenant(Map<String, Object> event, Context context) {
        ...
        //     System.out.println( String.format("Key : %s, value : %s", key, event.get(key)) );
        // }

        //Utils.logRequestEvent(event);
        APIGatewayProxyResponseEvent response;
        Map<String, String> params = (Map<String, String>) event.get("pathParameters");
        String tenantName = params.get("name");
        LOGGER.info("getOnboarding TenantName: {}", tenantName);
        Onboarding onboarding = dal.getOnboardingByTenantName(tenantName);
        
        
        // Auth_Info auth_info = new Auth_Info();
        // auth_info.aws_project_region = AWS_REGION;
        // auth_info.aws_cognito_region = AWS_REGION;
        // auth_info.aws_user_pools_id = onboarding.getUserPoolId();
        // auth_info.aws_user_pools_web_client_id = onboarding.getClientId();
        
        String auth = "{"
			+ "\"aws_project_region\":"+"\""+ AWS_REGION + "\", "
			+ "\"aws_cognito_region\":"+ "\""+ AWS_REGION + "\", "
			+ "\"aws_user_pools_id\":"+ "\""+ onboarding.getUserPoolId() + "\", "
			+ "\"aws_user_pools_web_client_id\":"+ "\""+ onboarding.getClientId() + "\""
			+ "}"
			;
         
        
        String authInfo = Utils.unescapeJson(auth);
        
        if (onboarding != null) {
            response = new APIGatewayProxyResponseEvent()
                    .withHeaders(CORS)
                    .withStatusCode(200)
                    .withBody(authInfo); //.withBody(Utils.toJson(onboarding)); 
        } else {
    ...
    public APIGatewayProxyResponseEvent insertOnboarding(Map<String, Object> event, Context context) {
        ...
        if (Utils.isBlank(onboardingRequest.getEmail())) {
            LOGGER.error("Onboarding request is missing Eamil");
            return new APIGatewayProxyResponseEvent()
                    .withStatusCode(400)
                    .withHeaders(CORS)
                    .withBody("{\"message\": \"Tier is required.\"}");
    ...
    protected void handleOnboardingTenantAssignedEKS(Map<String, Object> event, Context context) {
        ...
        if (OnboardingEvent.validate(event, "tenant")) {
            ...
            if (onboarding != null) {
                ...
                String tenantName = Objects.toString(onboarding.getRequest().getName(), "");
                String tenantSubdomain = Objects.toString(tenant.get("subdomain"), tenantName);
                String tier = Objects.toString(tenant.get("tier"), "default");
                String email = Objects.toString(onboarding.getRequest().getEmail(), "");
                ...
                templateParameters.add(Parameter.builder().parameterKey("TenantName").parameterValue(tenantName).build());
                templateParameters.add(Parameter.builder().parameterKey("AdminEmailParameter").parameterValue(adminEmailParameter).build());
                ...
                String templateUrl = "https://" + SAAS_BOOST_BUCKET + ".s3." + AWS_REGION
                        + "." + Utils.endpointSuffix(AWS_REGION) + "/tenant-eks-onboarding.yaml";
                String stackId;
                try {
    ...
    protected void handleOnboardingStackStatusChanged(Map<String, Object> event, Context context) {
     ...
     protected void handleOnboardingStackStatusChangedEKS(Map<String, Object> event, Context context) {
        // TODO stack events don't have the onboardingId, so we can't use OnboardingEvent::validate as written
        Map<String, Object> detail = (Map<String, Object>) event.get("detail");
        if (detail != null && detail.containsKey("tenantId") && detail.containsKey("stackId")
                && detail.containsKey("stackStatus")) {
                        ...
                        if (stack.isComplete()) {
                            if (stack.isBaseStack() && onboarding.baseStacksComplete()) {
                                if (stack.isCreated()) {
                                    ...
                                    for (Output output : appStack.outputs()) {
                                        LOGGER.info("Onboarding Cognito Stack Key:{} , Value:{}", output.outputKey(), output.outputValue());
                                        
                                        if("CognitoUsersUserPoolClientId".equals(output.outputKey()) ){
                                            onboarding.setClientId(output.outputValue());
                                        }else if("CognitoUsersUserPoolId".equals(output.outputKey())) {
                                            onboarding.setUserPoolId(output.outputValue());
                                        }
                                    }
                                    
                                    String tenantName = Objects.toString(onboarding.getRequest().getName(), "");
                                    
                                    onboarding.setTenantName(tenantName);
                                    ...
                                    // List<Parameter> parameters = new ArrayList<>();
                                    // Stack appStack = describeStacksResponse.stacks().get(0);
                                    // for (Output parameter : appStack.outputs() ) {
                                        
                                    //     LOGGER.info("Skipping onboarding status deleted for app OutputKey: {}, OutputValue: {}"
                                    //                 , parameter.outputKey(), parameter.outputValue());
                                    // }
    ...
    protected void handleOnboardingBaseProvisionedEKS(Map<String, Object> event, Context context) {
        if (Utils.isBlank(AWS_REGION)) {
                ...
                if (tenant != null) {
                    ...
                    templateParameters.add(Parameter.builder().parameterKey("Tier").parameterValue(tier).build());
                        ...
                        String stackName = "sb-" + SAAS_BOOST_ENV + "-tenant-" + tenantShortId + "-app-"
                                + "EKS" + "-" + Utils.randomString(12).toUpperCase();
                        ...
                        String templateUrl = "https://" + SAAS_BOOST_BUCKET + ".s3." + AWS_REGION
                                + "." + Utils.endpointSuffix(AWS_REGION) + "/tenant-eks-empty.yaml";//"/tenant-onboarding-app.yaml";
                        String stackId;
                        try {
                            CreateStackResponse cfnResponse = cfn.createStack(CreateStackRequest.builder()
                                    .stackName(stackName)
    ...
    protected void handleOnboardingProvisionedEKS(Map<String, Object> event, Context context) {
        ...
        if (OnboardingEvent.validate(event)) {
            ...
            if (onboarding != null) {
                ...
                //aws codepipeline tag-resource --resource-arn arn:aws:codepipeline:ap-northeast-2:033185771327:eks-saas-pipeline --tags key=Tenant,value=onboarding.getTenantId()
                
                // ListTagsForResourceResponse tagsResponse = codePipeline.listTagsForResource(r -> r.resourceArn("arn:aws:codepipeline:ap-northeast-2:033185771327:eks-saas-pipeline"));
                // String accountID = AWSSecurityTokenServiceClientBuilder.standard().build().getCallerIdentity(new GetCallerIdentityRequest()).getAccount();
                ...
                StsClient stsClient = StsClient.builder()
                                     ...
                                     .build();
                GetCallerIdentityResponse callerIdentity = stsClient.getCallerIdentity(getCallerIdentityRequest);;
                
                String pipelineArn = "arn:aws:codepipeline:"+ AWS_REGION +":"+callerIdentity.account() +":eks-saas-pipeline";
                codePipeline.getPipeline(r -> r.name("eks-saas-pipeline"));
                Tag tagTenant = Tag.builder().key("Tenant").value(onboarding.getTenantId().toString()).build();
                Tag tagName = Tag.builder().key("TenantName").value(Objects.toString(onboarding.getRequest().getName(), "")).build();

                TagResourceRequest tagRequest = TagResourceRequest.builder()
                            //.resourceArn("arn:aws:codepipeline:ap-northeast-2:033185771327:eks-saas-pipeline")
                            .resourceArn(pipelineArn)
                            .tags(tagTenant, tagName).build();
                
                // AddTagsToResourceRequest addTags = AddTagsToResourceRequest.builder()
                //     .resourceArn("arn:aws:codepipeline:ap-northeast-2:033185771327:eks-saas-pipeline")
                //     .tags(tag)
                //     .build();
                
                // tagsResponse.toBuilder().tags(tag).build();
                // tagRequest.tags().add(Tag.builder().key("Tenant").value(onboarding.getTenantId().toString()).build());
                
                codePipeline.tagResource(tagRequest);
                
                
                StartPipelineExecutionResponse response = codePipeline.startPipelineExecution(r -> r.name("eks-saas-pipeline"));
                LOGGER.info("Started tenant {} pipeline {} {}", onboarding.getTenantId(), "eks-saas-pipeline", response.pipelineExecutionId());
