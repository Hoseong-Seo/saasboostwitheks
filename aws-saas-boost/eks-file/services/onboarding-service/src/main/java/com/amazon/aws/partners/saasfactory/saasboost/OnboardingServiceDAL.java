...
public class OnboardingServiceDAL {
    ...
    public Onboarding getOnboardingByTenantId(String tenantId) {
        ...
        //EKS
    public Onboarding getOnboardingByTenantName(String tenantName) {
        final long startTimeMillis = System.currentTimeMillis();
        LOGGER.info("OnboardingServiceDAL::getOnboardingByTenantName");
        Onboarding onboarding = null;
        try {
            final int uuidLength = 36;
            String filter;
            if (tenantName.length() < uuidLength) {
                filter = "begins_with(tenant_name, :tenantName)";
            } else {
            ...
            ScanResponse scan = ddb.scan(ScanRequest.builder()
                    ...
                    .filterExpression(filter)
                    .expressionAttributeValues(
                            Collections.singletonMap(":tenantName", AttributeValue.builder().s(tenantName).build())
                    )
                    .build()
            );
            if (1 == scan.items().size()) {
                LOGGER.info("Scanning onboarding for tenant Name " + tenantName);
                onboarding = fromAttributeValueMap(scan.items().get(0));
            } else {
                LOGGER.info("Onboarding scan for tenant Name " + tenantName + " returned " + scan.items().size() + " results");
            }
        } catch (DynamoDbException e) {
            LOGGER.error("OnboardingServiceDAL::getOnboardingByTenantName " + Utils.getFullStackTrace(e));
            throw e;
        }
        long totalTimeMillis = System.currentTimeMillis() - startTimeMillis;

        LOGGER.info("OnboardingServiceDAL::getOnboardingByTenantName exec " + totalTimeMillis);
        return onboarding;
    }
    ...
    public static Map<String, AttributeValue> toAttributeValueMap(Onboarding onboarding) {
        ...
        //EKS
        if (onboarding.getClientId() != null) {
            item.put("client_id", AttributeValue.builder().s(onboarding.getClientId().toString()).build());
        }
        if (onboarding.getUserPoolId() != null) {
            item.put("user_pool_id", AttributeValue.builder().s(onboarding.getUserPoolId()).build());
        }
        if (onboarding.getTenantName() != null) {
        ...
        if (onboarding.getRequest() != null) {
            ...
            //EKS
            if (Utils.isNotBlank(request.getEmail())) {
                requestMap.put("email", AttributeValue.builder().s(request.getEmail()).build());
            }
            //
    ...
    public static Onboarding fromAttributeValueMap(Map<String, AttributeValue> item) {
        ...
        if (item != null && !item.isEmpty()) {
            ...
            if (item.containsKey("client_id")) {
                try {
                    onboarding.setClientId(item.get("client_id").s());
                } catch (IllegalArgumentException e) {
                    LOGGER.error("Failed to parse client_id from database: " + item.get("client_id").s());
                    LOGGER.error(Utils.getFullStackTrace(e));
                }
            }
            if (item.containsKey("user_pool_id")) {
                try {
                    onboarding.setUserPoolId(item.get("user_pool_id").s());
                } catch (IllegalArgumentException e) {
                    LOGGER.error("Failed to parse user_pool_id from database: " + item.get("user_pool_id").s());
                    LOGGER.error(Utils.getFullStackTrace(e));
                }
            }
            if (item.containsKey("tenant_name")) {
                try {
                    ...
                    LOGGER.error("Failed to parse tenant_name from database: " + item.get("tenant_name").s());
                    LOGGER.error(Utils.getFullStackTrace(e));
                }
            }
            //EKS
            ...
            if (item.containsKey("request")) {
                ...
                OnboardingRequest request = new OnboardingRequest(requestMap.get("name").s(), requestMap.get("email").s());
