...
echo "Input New API Gateway URL"
export SAAS_BOOST_PUBLIC_API=$(aws cloudformation list-stacks  --query "StackSummaries[*].StackName" --stack-status-filter CREATE_IN_PROGRESS CREATE_COMPLETE ROLLBACK_IN_PROGRESS ROLLBACK_FAILED ROLLBACK_COMPLETE DELETE_IN_PROGRESS DELETE_FAILED UPDATE_IN_PROGRESS UPDATE_COMPLETE_CLEANUP_IN_PROGRESS UPDATE_COMPLETE UPDATE_ROLLBACK_IN_PROGRESS UPDATE_ROLLBACK_FAILED UPDATE_ROLLBACK_COMPLETE_CLEANUP_IN_PROGRESS UPDATE_ROLLBACK_COMPLETE REVIEW_IN_PROGRESS | jq -r '.[]|select( startswith("sb-'$SAAS_BOOST_ENV'-publicapi"))')
APIGATEWAY_API=$(aws cloudformation describe-stacks --stack-name ${SAAS_BOOST_PUBLIC_API} | jq -r .Stacks[0].Outputs[0].OutputValue)
echo ${APIGATEWAY_API}
cp ./application/src/environments/environment.txt ./application/src/environments/environment.prod.ts 
sed -i -e 's,{{APIGATEWAY_API}},'$APIGATEWAY_API',g' ./application/src/environments/environment.prod.ts
...
echo $DOCKER_TAG
cd application
docker image build -t $CHOSEN_SERVICE --no-cache -f Dockerfile .
